...
public class LauncherModel extends BroadcastReceiver {
	 ...
	 */
	 ...
	 public void onReceive(Context context, Intent intent) {
		 ...
		 // add by steven
		 if( action.equals("com.jslauncher.launcher.AllApps2D.SORT"))
		 {
		 ...
		 {
			 final String packageName2 = intent.getData().getSchemeSpecificPart();
			 ...
			 Launcher.myframe.popwindow.myMessageHandler.sendMessage(m);
		 }
		*/
		 if (Intent.ACTION_PACKAGE_CHANGED.equals(action)
			 ...
			 if (Intent.ACTION_PACKAGE_CHANGED.equals(action) || action.equals("com.jslauncher.launcher.AllApps2D.SORT")) {
				 ...
				 if (!replacing) {
					 ...
					 // add by steven to save the install date			
					 SharedPreferences sharedAppDate = mApp.getSharedPreferences("appdate", Activity.MODE_PRIVATE);
					 SharedPreferences.Editor editor = sharedAppDate.edit();
					 editor.putLong(packageName, (new Date().getTime()));
					 editor.commit();
				 } else {
					 op = PackageUpdatedTask.OP_UPDATE;
				 }
	 ...
	 private class LoaderTask implements Runnable {
		...
		private void loadAndBindWorkspace() {
			 ...
			 mHandler.post(new Runnable() {
				 public void run() {
					 ...
					 if (callbacks != null) {
						 ...
						 ((Launcher) callbacks).addMyFolder();
					 }
				 }
			 });
		 ...
		 private void loadAllAppsByBatch() {
			 ...
			 SharedPreferences sharedAppDate = mApp.getSharedPreferences("appdate", Activity.MODE_PRIVATE);
			 SharedPreferences sharedAppCounter = mApp.getSharedPreferences("appcounter", Activity.MODE_PRIVATE);
			 ...
			 while (i < N && !mStopped) {
				 ...
				 */
				 ...
				 for (int j=0; i<N && j<batchSize; j++) {
					 // This builds the data and counter. add by steven
					 //  mAllAppsList.add(new ApplicationInfo(apps.get(i), mIconCache));
					 app  =   new ApplicationInfo(apps.get(i), mIconCache);
					 app.installData =  sharedAppDate.getLong(app.componentName.getPackageName(), 0);
					 app.useCounter =  sharedAppCounter.getInt(app.componentName.getPackageName(), 0);
	...
	private static String getLabel(PackageManager manager, ActivityInfo activityInfo) {
	  ...
	  *	增加按安装日期排序和按使用次数排序
	  */ 
	 public static final Comparator<ApplicationInfo> APP_DATA_COMPARATOR
	 = new Comparator<ApplicationInfo>() {
		 public final int compare(ApplicationInfo a, ApplicationInfo b) {
			 
			
			 int res1 = (b.installData - a.installData > 0?1:-1);
			 if (res1!=0)
				 return res1;
			 else 
				 return sCollator.compare(a.title.toString(), b.title.toString());
			 /*
			 int res =  -sCollator.compare(a.installData, b.installData);
			 if(0 == res)
				 return sCollator.compare(a.title.toString(), b.title.toString());
			 else 
				 return res;
	 ...
	 public static final Comparator<ApplicationInfo> APP_COUNTER_COMPARATOR
	 = new Comparator<ApplicationInfo>() {
		 public final int compare(ApplicationInfo a, ApplicationInfo b) {
			// int res = -sCollator.compare(a.useCounter, b.useCounter);
			 int res1 = b.useCounter - a.useCounter;
			 if (res1!=0)
				 return res1;
			 else 
				 return sCollator.compare(a.title.toString(), b.title.toString());
